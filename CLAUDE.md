...
### node ID를 프롬프트에 명시 (실행 에이전트 호출 전 필수 체크)
분석 에이전트 결과가 나오면, 실행 에이전트를 바로 호출하지 않는다.
반드시 다음 순서를 지킨다:
1. 분석 결과에서 node ID를 직접 추출해서 확인
2. 추출한 node ID를 실행 에이전트 프롬프트에 명시
3. 그 다음 실행 에이전트 호출

node ID 없이 실행 에이전트를 호출하면 figma-sync 에이전트가 거부한다.
```
