...
export const TopAppbarInstant = forwardRef<HTMLDivElement, TopAppbarInstantProps>(
  function TopAppbarInstant(props, ref) {
    ...
    if (variant === 'heading') {
      return (
        <div ref={ref} className={clsx('flex items-center h-full', className)}>
          <h1 className="typo-title-2 font-semibold text-current truncate">
            {props.title}
          </h1>
        </div>
      );
    ...
    if (variant === 'label') {
      return (
        <div ref={ref} className={clsx('flex items-center h-full', className)}>
          <span className="typo-title-2 font-semibold text-current truncate">
            {props.title}
          </span>
        </div>
      );
    ...
    if (variant === 'textButton') {
      return (
        <div ref={ref} className={clsx('flex items-center h-full', className)}>
          ...
          >
            <span className="typo-title-2 font-semibold text-current">
              {props.label}
            </span>
            <ChevronDownIcon />
          </button>
